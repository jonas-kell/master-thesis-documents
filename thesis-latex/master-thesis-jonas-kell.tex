\documentclass[
headings=optiontohead,              % allows double headers
12pt,                               % fontsize 
DIV=14,                             % koma script diveider amount. tells koma how much of the site can be written to
twoside=true,                       % if set to true, automatically formats as book style with different left and right pages
open=any,                           % starting page on twosided texts 
BCOR=07mm,                          % correction that accounts for the center of the pages being glued in
toc=bibliographynumbered            % bibliography gets a number and is listed in the table of contents
]{scrreport}

\input{./../shared-latex/packages}  % required packages

\title{Classical networks for the Hubbard model with a tilted potential}
\author{Jonas Kell}
\date{13$^\text{th}$ February 2025}
\newcommand{\pdfkeywords}{neural networks, ai, physics, quantum mechanics, machine learning, analytical calculations, classical networks, hubbard model}
\newcommand{\pdfsubject}{Master Thesis}

\input{./../shared-latex/config}                % another file that holds the package/document configuration
\input{./../shared-latex/format}                % another file that holds format information
\input{./../shared-latex/commands}              % another file that holds predefined commands
\input{./../shared-latex/hyphenation}           % another file that holds predefined hyphenation helpers for unknown words


\begin{document}

% ! Bibliography, page numbering and Title setups
\thispagestyle{empty}                           % make sure title page is not numbered or anything else
\input{titlepage}                               % include title-page
\cleardoublepage                                % make sure, that if double-page is active, to reset the double page counter
\pagestyle{scrheadings}                         % puts current chapters title into the header in small gray font
\pagenumbering{roman}                           % number the pages of the table of contents in roman numerals
\renewcommand{\contentsname}{Table of Contents} % title of table of contents
\tableofcontents                                % table of contents
\input{lists}                                   % list of abbreviations, figures, etc
\cleardoublepage                                % make sure, that if double-page is active, to reset the double page counter
\pagenumbering{arabic}                          % number the pages of the main document in Arabic numerals

% After this, the redefinition of the "Kolumnentitel" takes place
\clearpairofpagestyles
\ihead{\leftmark}
\ohead{\Ifstr{\leftmark}{\rightmark}{}{\rightmark}}
\cfoot*{\pagemark}
% End of the "Kolumnentitel" redefinition

%  Continuously number equations (basically all equations are in chapter 2, that makes it unnecessary and if we have over 100 equations anyway, it is cleaner to just number them through globally)
\renewcommand{\theequation}{\arabic{equation}} % Use global numbering

% ! Main Document Body
\chapter{Introduction}
\label{sec:introduction}
\input{./auxillary/introduction}
\FloatBarrier

\chapter{Theory}
\label{sec:theory}
\input{./theory/pre-word.tex}
\FloatBarrier

    \section{Physical Basics}
    \label{sec:theory-physics}
    \input{./theory/introduction.tex}
    \FloatBarrier
        \subsection{Different Particle-Types, their Operators and Mappings}
        \label{sec:particles}
        \input{./theory/particles/particles.tex}
        \FloatBarrier

        \subsection{Hubbard Model and discussed Hamiltonian}
        \label{sec:theory-hubbard-hamiltonian}
        \input{./theory/hubbard/hubbard.tex}
        \FloatBarrier

        
        \subsection{Interaction Picture and Time Evolution}
        \label{sec:theory-interaction-picture}
        \input{./theory/interaction-picture/interaction-picture.tex}
        \FloatBarrier
        
        \subsection{Operators in the Interaction Picture}
        \label{sec:theory-approximation-evaluation}
        \input{./theory/interaction-picture/approximation-evaluation.tex}
        \FloatBarrier

    \section{Observable Measurements}
    \label{sec:theory-observables}
    \input{./theory/observable-measurement/introduction.tex}
    \FloatBarrier

        \subsection{Observable Sampling}
        \label{sec:theory-observables-sampling}
        \input{./theory/observable-measurement/observable-measurement.tex}
        \FloatBarrier

        \subsection{Occupation Types}
        \label{sec:theory-observables-occupations}
        \input{./theory/observable-measurement/occupations.tex}
        \FloatBarrier

        \subsection{Spin-Current}
        \label{sec:theory-observables-current}
        \input{./theory/observable-measurement/current.tex}
        \FloatBarrier

        \subsection{Reduced Density Matrix}
        \label{sec:theory-observables-density-matrix}
        \input{./theory/observable-measurement/density-matrix.tex}
        \FloatBarrier
        
        \subsection{Entanglement Measurements}
        \label{sec:theory-observables-entanglement}
        \input{./theory/observable-measurement/entanglement.tex}
        \FloatBarrier
        
        \subsection{Energy}
        \label{sec:theory-observables-energy}
        \input{./theory/observable-measurement/energy.tex}
        \FloatBarrier

    \section{Variational Classical Networks}
    \label{sec:theory-variational-classical-networks}
    \input{theory/variational-classical-networks/introduction.tex}
    \FloatBarrier
        
        \subsection{General Theory of VCN}
        \label{sec:theory-variational-classical-networks-basics}
        \input{theory/variational-classical-networks/general-vcn.tex}
        \FloatBarrier


        \subsection{Application to this Task}
        \label{sec:theory-variational-classical-networks-application}
        \input{theory/variational-classical-networks/application-vcn.tex}
        \FloatBarrier

        
        \subsection{Explicit Time-Dependency}
        \label{sec:theory-variational-classical-networks-time-dependency}
        \input{theory/variational-classical-networks/expl-time-dependency.tex}
        \FloatBarrier


    \section{Time-Complexity Optimizations}
    \label{sec:theory-optimizations}
    \input{./theory/optimizations/introduction.tex}
    \FloatBarrier

        \subsection{Monte Carlo Sampling}
        \label{sec:theory-optimizations-monte-carlo}
        \input{./theory/optimizations/mc-sampling.tex}
        \FloatBarrier

        \subsection{Analytical Simplifications for Special Cases}
        \label{sec:theory-optimizations-analytical}
        \input{./theory/optimizations/special-case-simplifications.tex}
        \FloatBarrier

        \subsection{Geometry-Dependent Interaction-Range}
        \label{sec:theory-optimizations-geometry}
        \input{./theory/optimizations/geometry.tex}
        \FloatBarrier
        

\chapter{Implementation and Calculation Details}
\label{sec:implementation-details}
\input{implementation/introduction.tex}
\FloatBarrier

    \section{Math-Manipulator}
    \label{sec:implementation-details-math-manipulator}
    \input{implementation/math-manipulator.tex}
    \FloatBarrier

    \section{Python-Script Generation}
    \label{sec:implementation-details-script-generation}
    \input{implementation/script-generation.tex}
    \FloatBarrier

    \section{Programming Style Optimizations}
    \label{sec:implementation-details-style-optimizations}
    \input{implementation/style-optimizations.tex}
    \FloatBarrier

    \section{Simplification Verifications}
    \label{sec:implementation-details-simplification-verifications}
    \input{implementation/simplification-verifications.tex}
    \FloatBarrier

\chapter{Numerical Experiments}
\label{sec:experiments-numerical}
\input{experiments/introduction.tex}
\FloatBarrier

    \section{Time-Complexity Verification}
    \label{sec:experiments-time-complexity}
    \input{experiments/time-complexity.tex}
    \FloatBarrier

    \section{Perturbation-Approximation Convergence}
    \label{sec:experiments-perturbation-convergence}
    \input{experiments/perturbation-convergence.tex}
    \FloatBarrier

    \section{Monte-Carlo-Sampling Convergence}
    \label{sec:experiments-mc-convergence}
    \input{experiments/mc-convergence.tex}
    \FloatBarrier

    \section{Insufficient Variational Ansatz}
    \label{sec:experiments-vcn-fails}
    \input{experiments/vcn-fails.tex}
    \FloatBarrier
    
    \section{Improvements through VCN}
    \label{sec:experiments-vcn-successful}
    \input{experiments/vcn-successful.tex}
    \FloatBarrier

    \section{System-Size Dependency of Measurements}
    \label{sec:experiments-size-dependency-measurement}
    \input{experiments/size-dependency-measurement.tex}
    \FloatBarrier

\chapter{Conclusion}
\label{sec:conclusion}
\input{./auxillary/conclusion}
\FloatBarrier

% ! Addendum
\input{./../shared-latex/bibliography}
\input{appendix/appendix}
\end{document}





