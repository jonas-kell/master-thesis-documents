...
In \autoref{eq:transition-probability-final} the \emph{acceptance-ratio} $\alpha$ - that is necessary for the Metropolis-Hastings-Algorithm to work - is derived (as per \cite{metropolisHastingsAlgorithmGeneral}) and decide on the transition from state \ketN[N] to \ketN[{\vphantom{N}\smash{\tilde{N}}}].
